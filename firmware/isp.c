...
void ispSetSCKOption(uchar option) {
	...
	if (option == USBASP_ISP_SCK_AUTO)
		option = USBASP_ISP_SCK_DEFAULT;

	/* Set here, crush later */
	prog_sck = option;

	if (option >= USBASP_ISP_SCK_HW_MIN) {
		...
		case USBASP_ISP_SCK_3000:
			/* enable SPI, master, 3.0MHz, XTAL/4 */
...
uchar ispEnterProgrammingMode() {
	...
	while (prog_sck >= USBASP_ISP_SCK_MIN) {
		uchar count = 32;
